...
export const meta = {
    ...
    labels: ["aquaculture", "policy", "algorithms"]
}
...
Objectives are to lower expenses, and fix or increase price. New sites have a mean frequency (<Eqn text={"\\lambda"}/>, y<sup>-1</sup>), 
...
Given a window of size <Eqn text={"A_0"}/>, and a set of <Eqn text={"N"}/> existing leases, no viable space will remain 
in jurisdiction area <Eqn text={"A"}/> after <Eqn text={"t=A(41.66A_0-N)(\\lambda A_0)^{-1}"}/>. 
...
by year 3 (table below). Mean <Eqn text={"t"}/> is 15 years, without considering competing uses or closures. Less than 22% allocation, 
...
| Year |<Eqn text={"N"}/>|<Eqn text={"\\lambda"}/>|<Eqn text={"t"}/>|<Eqn text={"min(t)"}/>|
|---|---|---|---|---|
...
Area incentives like credits or subsidies are structured at or above the municipal level—changing <Eqn text={"\\lambda"}/> within a 
