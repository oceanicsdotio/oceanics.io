...
const WELL_KNOWN_TEXT = (0, fs_1.readFileSync)("functions/lexicon/words.txt")
    .toString()
    .split("\n");
/**
 ...
 * @param {*} param0
 */
const search = ({ words, pattern, maxCost, }) => words.reduce((result, word) => {
    const cost = [...word].reduce((row, symbol) => calculateRow({
        previous: row,
        word: pattern,
        symbol
    }), Array(Array(pattern.length + 1).keys())).pop();
    if (cost <= maxCost)
 ...
 * @param {*} param0
 */
const trie = ({ words = [], root = {}, encode = (weight) => weight + 1, initialWeight = 1 }) => words.reduce((root, pattern) => {
    let node = root;
    [...pattern].forEach(c => {
 ...
 * @param {*} param0
 */
function recurse({ node, pattern, maxCost, symbol = "", previous = null, }) {
    ...
    const descend = Math.min(...row) <= maxCost &&
        typeof node.children === "object";
    // cost of child words
    const children = !descend ? [] :
        Object.entries(node.children)
            .map(([symbol, node]) => recurse({
            node,
            pattern,
            maxCost,
            symbol,
