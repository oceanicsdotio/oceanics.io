...
const simulation = () => {
    ...
    for (let ii=0; ii < n; ii++) {
        const [x, y, z] = e[ii].coordinates;
        const scale = (1.0 - 0.9*z) * state.radius;
        ctx.globalAlpha = 1.0 / Math.log2(e.length);


        for (let [jj, link] of Object.entries(e[ii].links)) {
            ...
            const s =  scale / e[ii].links[jj].spring.x;

            link.vec = link.vec.map((k, index) => {
                const delta = k / d * f / e.length;
                e[ii].velocity[index] += delta;
                e[jj].velocity[index] -= delta;
                return e[ii].coordinates[index] - e[jj].coordinates[index];
            });
            ...
            const [bx, by, bz] = [
                e[jj].coordinates[0] * canvas.width + link.vec[0] * s,
                e[jj].coordinates[1] * canvas.height + link.vec[1] * s,
                e[jj].coordinates[2] * state.depth + link.vec[2] * s
            ];
        ...
        ctx.beginPath();
        ctx.moveTo(
            e[ii].coordinates[0] * canvas.width,
            e[ii].coordinates[1] * canvas.height
        );
        ctx.lineTo(
            e[ii].coordinates[0] * canvas.width + e[ii].heading[0] * scale,
            e[ii].coordinates[1] * canvas.height + e[ii].heading[1] * scale
        );
        ctx.stroke();

        // Update positions
        let speed = magnitude(e[ii].velocity);
        state.energy.kinetic += 0.5*speed*speed;

        e[ii].coordinates = e[ii].coordinates.map((x, kk) => {
            e[ii].velocity[kk] += state.cursor[kk] * state.torque * (0.5 - z);
            x += e[ii].velocity[kk];
            if (x > 1.0 - state.padding) {
                x -= 2*(x - 1.0 - state.padding);
                e[ii].velocity[kk] *= -state.bounce;
            } else if (x < state.padding) {
                x -= 2*(x - state.padding);
                e[ii].velocity[kk] *= -state.bounce;
            }
            e[ii].heading[kk] = speed > 0.00001 ? e[ii].velocity[kk] / speed : e[ii].heading[kk];
            return x
        });
    ...
    ctx.putImageData(encode(0.5, 0.0), 100, 100);
