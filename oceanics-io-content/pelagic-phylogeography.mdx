...
published: "2012-12-10T12:00:00.000Z"
description: |
  Phylogeography of pelagic fishes at the Gulf of Mexico / Atlantic boundary
labels: [fisheries, genetics, geography, gulf of mexico]
authors: [Keeney]
references:
...
<Equation text={"F_{st} = {1 \\over {(1 + 4Nm)}}"}/>

Values <Equation text={"F_{st}"}/> near zero indicate population homogeneity, while values near one indicate heterogeneity. The relationship between the three variables is shown below in figure 1. Increasing either m or N independently will increase <Equation text={"F_{st}"}/>.


The value of <Equation text={"m"}/> is usually obtained from field experiments following individuals, since $N$ is impossible to estimate from readily available catch data. <Equation text={"N"}/> is then an abstraction of the population’s reproductive capability useful for theoretical comparison [2]. It is always less than the actual population size (Tringali and Bert 1998), due to variable fecundity and success of individuals. 

In studies utilizing mtDNA, effective population size is the number of females (<Equation text={"N_f"}/>), usually assumed to be half of the whole population (Bowen and Avise 1990).

<Equation text={"Nm"}/>
is itself considered a single variable for the number of migrating individuals per generation. A single individual per generation (<Equation text={"Nm=1"}/>) is a critical point after which migration will always, eventually, overpower intrapopulation mutation (Grosberg and Cunningham 2001; [2]. This simplification assumes that the populations are immune to mutation and selection (Neigel 2001; Wright 1951). 
