...
const register = async ({ email, password, secret, apiKey }) => {
    ...
    let message;
    try {
        records = transform((await (0, shared_1.connect)(cypher.query)).records);
    }
    catch {
        records = [];
    }
...
const getToken = async ({ email, password, secret }) => {
    ...
    const records = transform((await (0, shared_1.connect)(node.load().query)).records);
    let statusCode;
    let body;
    if (records.length !== 1) {
        statusCode = 403;
...
const manage = async ({ token, email, password }) => {
    const claim = jsonwebtoken_1.default.verify(token, process.env.SIGNING_KEY);
