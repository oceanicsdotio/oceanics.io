...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function() {
    navigator.serviceWorker.register('../utils/service-worker.js').then(function(registration) {
...
const loadGeoJSON = async (map, layers) => {
    return await Promise.all(layers.map(async ({render, behind}) => {
        render.source = {
            'type': 'geojson',
            'data': await fetch("../../static/" + render.id + ".json").then(r => r.json()),
            'generateId': true,
        };
        map.addLayer(render);
...
.then(r => r.json())
.then(style => {
    console.log("Loaded and deserialized style config JSON.");
...
.then(map => {
    ...
    map.on('load', async () => {
        const layers = await fetch("../../static/layers.json").then(r => r.json());
