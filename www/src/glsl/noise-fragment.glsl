...
#define TIME_SCALE 0.00025
#define AMPLITUDE 0.5  // 0.5 is default, higher saturates
#define LACUNARITY 2.8  // 2.0 is default, lower is smooth, higher introduces speckling
#define GAIN 0.4
...
float book_of_shaders_pattern( in vec2 st, float time)
{
    vec2 q = vec2(
        fbm(st), 
        fbm(st + 1.0)
...
void main() {
    ...
    vec2 st = gl_FragCoord.xy / vec2(32, 32) * 3.0;
    float time = u_time * TIME_SCALE;

    float f = quilez_pattern(st, time);

    gl_FragColor = vec4(f*0.8, f*0.9, 0.8*f, f);
}