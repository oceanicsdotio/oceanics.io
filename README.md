...
Deploy just the database with `docker-compose up neo4j`
...
We use markers, keywords, and dependencies to filter for tests. The custom markers are described in `pytest.ini`. The setup and utility functions for `pytest` are in a file named `conftest.py`. 

The command for the default tests is `$(bathysphere --kw=KEYWORD_STRING)`.

If you get permission errors, pytest is crawling a directory in use by Neo4j. This should be ignored by default in the commands generated by the CLI.
